...
| 9 Purchase accounting | Purchase orders/receiving plus purchase bills, supplier payable ledger, payments, debit notes and reachable Purchase Bills portal | Purchase returns with stock reversal, supplier statement/export and physical supplier-payment acceptance remain |
| 10 Expenses/accounting | Expense categories/entries, idempotent branch expense posting, payment-mode/category summaries and reachable Expenses portal | Full double-entry chart of accounts, receivables/payables consolidation and reconciled profit statements remain |
...
`purchase_bills`, `purchase_bill_items`, `supplier_ledger_entries`, and `purchase_debit_notes` are migrated in `20260920_0022` and included in company/branch scope enforcement. Purchase bills are financially posted independently from PO receiving, use Decimal totals, are idempotent, and create supplier payable debits. Payment and debit-note endpoints enforce outstanding-balance limits, update bill status/balance atomically, and write supplier ledger credits. The Retail portal now exposes a Purchase Bills form. `backend/tests/test_purchase_accounting.py` covers bill idempotency, payable balance, payment, debit note, and supplier ledger entries. Purchase returns/stock reversal and physical supplier-payment acceptance remain target-Hub gates.

### Phase 10 evidence

`expense_categories` and `expense_entries` are migrated in `20260920_0023` and included in company/branch scope enforcement. Admins can create categories; admins and store managers can post idempotent, Decimal-valued branch expenses. `/api/expenses/summary` returns reconciled totals by category and payment mode for a date range, with audit logs for every write. The Retail portal now exposes an Expenses and Cashbook form. `backend/tests/test_expenses.py` covers category creation, duplicate-safe posting, and summary totals. Full double-entry accounting and external payment settlement remain later gates.
